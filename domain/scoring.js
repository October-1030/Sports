/**
 * 运动天赋评估评分引擎 - JavaScript版本
 * 纯函数实现，输入 AssessmentInput，输出 AssessmentResult
 * 与TypeScript版本保持完全一致的逻辑
 */
...
if (typeof require !== 'undefined') {
  // Node.js环境
  ScoringConfig = require('./scoring.config.js');
} else if (typeof window !== 'undefined' && window.ScoringConfig) {
  // 浏览器环境
  ScoringConfig = window.ScoringConfig;
} else {
  throw new Error('无法加载评分配置，请确保正确引入 scoring.config.js');
}
...
/**
 * 运动天赋评估核心函数
 * @param {Object} input - 评估输入数据 (AssessmentInput类型)
 * @param {Object} [config={}] - 评分配置（可选，使用默认配置）
 * @returns {Object} 评估结果 (AssessmentResult类型)
 */
...
function calculateDetails(input, cfg) {
  return {
...
// 遗传潜力详细得分
function calculateGeneticDetails(input, cfg) {
...
// 当前能力详细得分  
function calculateCurrentDetails(input, cfg) {
...
// 专项技能详细得分
function calculateSpecialtyDetails(input, cfg) {
...
// 身体优势详细得分
function calculatePhysicalDetails(input, cfg) {
...
// 心理特征详细得分
function calculatePsychologyDetails(input, cfg) {
...
// 身高基因得分
function calculateHeightGenesScore(family, cfg) {
  let score = 0;
...
// 父母运动基因得分
function calculateParentSportsScore(parents, cfg) {
  let score = 0;
...
// 基础技能得分
function calculateBasicSkillsScore(basicSkills, cfg) {
  const skills = ['run', 'jump', 'throw', 'climb', 'balance'];
  let totalScore = 0;
...
// 兴趣广度得分
function calculateInterestsScore(interests, cfg) {
  // 兴趣项目数量得分，每个兴趣2分，上限25分
  return Math.min((interests || []).length * 2, 25);
}

// 水上项目得分
function calculateAquaticScore(aquatic, interests, cfg) {
  let score = 0;
...
// 球类项目得分
function calculateBallScore(ballSkills, interests, cfg) {
  let score = 0;
...
// 田径项目得分  
function calculateTrackScore(trackSkills, basicSkills, interests, cfg) {
  let score = 0;
  
  // 基础技能相关性加分（跑、跳、投掷）
  const relevantSkills = ['run', 'jump', 'throw'];
...
// 技巧项目得分
function calculateTechScore(techSkills, highlights, parents, cfg) {
  let score = 0;
...
// 身体优势得分
function calculateStrengthsScore(strengths, cfg) {
  // 每项身体优势10分
  return (strengths || []).length * 10;
}

// 健康状况得分（负分项）
function calculateHealthScore(health, cfg) {
  let penalty = 0;
...
// 性格特质得分
function calculateTraitsScore(traits, cfg) {
  let score = 0;
...
function calculateDimensionScores(details, cfg) {
...
function calculateOverallScore(scores, cfg) {
  const overall = 
...
function determinePathway(overall, cfg) {
...
function recommendSports(input, scores, details, cfg) {
...
function generateRecommendations(scores, pathway, input, cfg) {
  const recommendations = [];
...
/**
 * 格式化评估结果为可读文本
 * @param {Object} result - 评估结果 (AssessmentResult类型)
...
/**
 * 验证输入数据完整性
 * @param {Object} input - 输入数据 (部分AssessmentInput)
 * @returns {Object} 验证结果和缺失字段
 */
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    validateInput,
    // 为测试导出的函数
    calculateScore: evaluate,  // 别名
    determinePath: determinePathway,
    assessSportsSuitability: recommendSports,
    // 内部函数导出（仅供测试）
    calculateDetails,
